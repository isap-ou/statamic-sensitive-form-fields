...
class DecryptExistingCommand extends Command
{
    protected $signature = 'sensitive-fields:decrypt-existing
                            {--form= : Only process submissions from this form handle}
                            {--dry-run : Preview changes without persisting}';

    protected $description = '[PRO] Decrypt encrypted values of sensitive fields in existing form submissions.';
            ...
            foreach ($submissions as $submission) {
                ...
                try {
                    foreach ($sensitiveHandles as $field) {
                        ...
                        // decrypt() returns the raw value when decryption fails — skip with error.
                        if ($decrypted === $value) {
                            $totalErrors++;
                            $this->warn("Could not decrypt [{$field}] in submission [{$submission->id()}].");

                            continue;
                        }

                        if (! $isDryRun) {
                            $submission->set($field, $decrypted);
                        }
